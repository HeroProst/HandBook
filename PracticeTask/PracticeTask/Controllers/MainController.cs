	...
	public class MainController : Controller
	{
        ...
        public IActionResult Report(string selectOption = "", string selectValue = "")
        {
            ...
            if(numSelectionValue == null)
                sql = $"SELECT Id, Firstname, Secondname, Patronymic, id_Department, id_Cabinet, Phone FROM td_Employes where {selectOption} Like '%{selectValue}%'";
            else
                sql = $"SELECT Id, Firstname, Secondname, Patronymic, id_Department, id_Cabinet, Phone FROM td_Employes where {selectOption} = '{numSelectionValue}'";

            if(selectOption == "" || selectValue == "")
                sql = "SELECT Id, Firstname, Secondname, Patronymic, id_Department, id_Cabinet, Phone FROM td_Employes";
		...
		public IActionResult AuthorizationPage(string login, string pass, bool UnRegistred = false)
        {
            ...
            string sql = $"SELECT Id, UserLogin, UserPassword FROM UserAuthorization WHERE UserLogin = '{login}' AND UserPassword = '{pass}'";
        ...
        public IActionResult AddEmployee(string firstname, string secondname, string patronymic, string phone, string department, string cabinet)
        {
            ...
            string sql = $"INSERT INTO td_employes (firstname,secondname,patronymic,id_department,id_cabinet,phone)" +
        ...
        public IActionResult EditEmployee(string firstname, string secondname, string patronymic, string phone, string department, string cabinet, string employee)
        {
            ...
            string sql = $"Update td_employes Set firstname = '{firstname}', secondname = '{secondname}', patronymic = '{patronymic}',id_department = {departmentId}, id_cabinet = {cabinetId},phone = '{phone}'" +
                $"WHERE id = {employeId}";
        ...
        public IActionResult DeleteEmployee(string employe)
        {
            ...
            string sql = $"DELETE FROM td_employes where id = {employeId}";
        ...
        public IActionResult DeleteDepartment(string department)
        {
            ...
            string sql = $"DELETE FROM td_employes where id_department = {departmentId}";
